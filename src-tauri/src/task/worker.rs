...
impl TaskWorker {
    ...
    async fn process_task(&self, task: QueuedTask) -> Result<()> {
        ...
        let result = match task.task_type.as_str() {
            ...
            "ai_design_character" => handlers::text::handle_ai_design_character(&task).await,
            "ai_design_location" => handlers::text::handle_ai_design_location(&task).await,
            "ai_modify_appearance" => handlers::text::handle_ai_modify_appearance(&task).await,
            "ai_modify_location" => handlers::text::handle_ai_modify_location(&task).await,
            "ai_modify_shot_prompt" => handlers::text::handle_ai_modify_shot_prompt(&task).await,
